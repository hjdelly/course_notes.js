...
// a fucntion allows you to encaspulate behavior
var str = 'The quick brown fox jumped over the lazy dog';

str.length(); // VM2349:1 Uncaught TypeError: str.length is not a function-length is not a function
...
str.charAt(2); // "e" -whatever element is at index of 2-ZERO INDEX-starts with zero (T=0 H=1 E=2 in the word The)

str.charAt(200); // "" -this is returned if the index doesn't exist
...
str.includes('quick'); // true //to check if any values are included in the string use includes
                  
str.endsWith('dog'); // true // checks end of string-it checks for the characters
                 
str.startsWith('Foo'); // false no Foo // startswith just looks at the very front and checks if what we pass in starts at the point
//////////////////////////////////////////////////////////////////////
...
'555-555-5555'.match(/((\(\d{3}\) ?)|(\d{3}-))?\d{3}-\d{4}/) // (4) ["555-555-5555", "555-", undefined, "555-", index: 0, input: "555-555-5555"]0: "555-555-5555"1: "555-"2: undefined3: "555-"index: 0input: "555-555-5555"length: 4__proto__: Array(0)
                ...
                // if you want to replace something-replace takes in 2 arguments the word you want to replace and then the word you replace with
...
str.indexOf('jumped'); // 20-looks through whole string-and returns first found at the index
...
str2.lastIndexOf('again'); // 54 -same as above just finds the last word and returns that index
